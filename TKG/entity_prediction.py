...
from utils import load_quads, \
    myConvert, cal_metrics, CosineEmbeddingLoss, \
    load_history_paths, reshape_ranking_data, load_entities,load_relations
from torch.cuda.amp import GradScaler, autocast
from transformers import set_seed
from models import TKGModel
import argparse
...
parser.add_argument('--learning_rate', type=float, default=1e-5,
                    help='learning rate')
parser.add_argument('--model', type=str, default='../model/BERT',
                    help='sentence transformer model name on Hugging Face website (huggingface.co)')
parser.add_argument('--tokenizer', type=str, default='../model/BERT',
...
if args.path_dir is None:
    path_dir = os.path.join("../data/path_data/", args.dataset)
else:
    path_dir=args.path_dir

if args.model_load_file is None:
    model_load_file=os.path.join(f"../save/{args.dataset}",f"entity_prediction_{args.mode}/3_best_val.pth")
else:
...
if args.output_dir is None:
    output_dir=os.path.join(f"../output/{args.dataset}", f"entity_prediction_{args.mode}/")
else:
...
entity_text = load_entities(data_dir,entity_dict)
relation_text = load_relations(data_dir,relation_dict)
...
# ranking_quads = ranking_quads[:42000]
...
model = TKGModel(tokenizer_name=args.tokenizer,model_name=args.model,num_ents=len(entity_text),num_rels=data.num_rels,device=device)

if args.do_train:
    model.init_parameter(entity_text,relation_text)

# model=SentenceTransformer(tokenizer_name=args.tokenizer,model_name=args.model,device=device)
# model = TKG_Model(num_ents=len(entity_dict),num_rels=len(relation_dict),device=0,h_dim=200)
